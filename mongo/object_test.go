...
func Test_ObjectCreate(t *testing.T) {
	...
	t.Logf("now count:%d", n)

	// test limit
	if n, err = m.Objects().Filter(orm.M{
		"age": orm.TagValGte + "17",
	}).Count(); err != nil {
		t.Error(err.Error())
		return
	} else {
		t.Logf("age gte 17 count:%d", n)
	}

	// test time query
	if n, err = m.Objects().Filter(orm.M{
		"birthday": orm.TagValLt + orm.PubTimeQueryFormat(time.Now()),
	}).Count(); err != nil {
		t.Error(err.Error())
		return
	} else {
		t.Logf("query time count:%d, %s", n, orm.PubTimeQueryFormat(time.Now()))
	}
}
